...
def Rework():
  ...
  "jurisdiction": "AU"
    ...
    #print(Case_Id)
    if (Case_response['type']) == 'Rework':
            assert (Case_response['type']) == 'Rework'
...
def Update_RiskLevel_Low():
    ...
    Case_response = response.json()
    if (Case_response['riskLevel']) == '"Low"':
        assert (Case_response['riskLevel']) == '"Low"'

Update_RiskLevel_Low()

def Update_RiskLevel_Standard():
    url2 = New_url
    ...
    Case_response = response.json()
    if (Case_response['riskLevel']) == '"Standard"':
        assert (Case_response['riskLevel']) == '"Standard"'

Update_RiskLevel_Standard()

def Update_RiskLevel_Hightened():
    url2 = New_url
    print("Patch_url: "+url2)
    data2= {
          "legalEntityName": "CORS Security",
    ...
    Case_response = response.json()
    if (Case_response['riskLevel']) == '"Hightened"':
        assert (Case_response['riskLevel']) == '"Hightened"'

Update_RiskLevel_Hightened()
